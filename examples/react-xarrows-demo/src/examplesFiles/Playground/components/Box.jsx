...
const Box = (props) => {
  ...
  const handleClick = (e) => {
    ...
    if (props.actionState === "Normal") {
      ...
      props.setLines((lines) => [
        ...lines,
        {
          props: { startPoint: props.selected.id, endPoint: props.box.id },
          menuWindowOpened: false,
        },
      ]);
    } else if (props.actionState === "Remove Connections") {
      props.setLines((lines) => lines.filter((line) => !(line.root === props.selected.id && line.endPoint === props.box.id)));
    }
  };
  ...
  if (props.selected && props.selected.id === props.box.id) {
    ...
    (props.actionState === "Add Connections" &&
      // props.sidePos !== "right" &&
      props.lines.filter((line) => line.root === props.selected.id && line.endPoint === props.box.id).length === 0) ||
