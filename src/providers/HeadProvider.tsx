...
const {
  ...
  useProvider: useHeadProvider,
  useProviderRegister: useHeadProviderRegister,
} = createProvider<HeadProviderVal, HeadProviderValPrepared>("HeadProvider", {
  prepareValue: (val) => {
    const res = produce(val, (draft) => {
      if (draft.pos) draft.pos = new Vector(draft.pos);
      if (draft.dir) draft.dir = new Dir(draft.dir);
    });
    return res as HeadProviderValPrepared;
  },
});

export { HeadProvider, useHeadProvider, useHeadProviderRegister };
...
//   // console.log("finalVal.dir", finalVal.dir);
//   return (
