...
const round = (num: number, level = MATH_PRECISION) => {
...
const operatorFunc = (p: Vector, p2: Vector | number, operator) => {
  ...
  if (isNaN(v.x)) v.x = 0;
  if (isNaN(v.y)) v.y = 0;
  // handle very annoying javascript precision point shit
  // v.x = round(v.x);
  // v.y = round(v.y);
  return v;
};
...
export class Vector {
  ...
  add(p: Vector | number) {
    return operatorFunc(this, p, operators.add);
  }

  sub(p: Vector | number) {
    return operatorFunc(this, p, operators.sub);
  }

  mul(p: Vector | number) {
    return operatorFunc(this, p, operators.mul);
  }

  dev(p: Vector | number) {
    return operatorFunc(this, p, operators.dev);
  }
...
export class Line {
  ...
  constructor(
    ...
    { startIncluded = true, endIncluded = false } = { startIncluded: true as boolean, endIncluded: false as boolean }
  ) {
    this.root = start;
    this.end = end;
    let [a, b] = this.getAB();
