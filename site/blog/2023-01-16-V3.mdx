...
this approach does not scale well as complexity grows, and worse, this way the component can't be extended easily.
...
import { XArrow, useXArrow, autoAnchor, XArrowEnd, NormalizedGSvg,XLine,AutoSelectAnchor } from "react-xarrows"
...
export const Head = ({ color, size }) => {
  ...
  return (
    <XArrowEnd pos={{ x: x2, y: y2 }} dir={{ x1: x2 - x1, y: y2 - y1 }} size={size}>
      <NormalizedGSvg>
